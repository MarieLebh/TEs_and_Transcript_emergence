...
"""
Filter de novo transcripts for TPM and splicing
"""
...
def get_de_novo_positions(df, ids):
    df1 = df.query('Transcript_ID in @ids')
    return df1
...
outfile = "AK5_final_de_novo_ids.txt" #name of the outfile containing the final de novo ids to use

print(Population)
#Get all gene and transcript ids from the gtf file

x = get_gene_id(GTF)
print("Number of transcripts original gtf: ", len(x))

#Add a column with the number of alternative spliceforms for each transcript
df = return_occurence(x)

#Get a list of all de novo transcript ids
ids = get_de_novo_ids(de_novo_ids)

#Creates a subsetted df keeping only the de novo transcripts
df1 = get_de_novo_positions(df, ids)

#Again count the number of alternative spliceforms for each transcript
df1 = return_occurence2(df1)

#Make a new column with the difference of the number of alternative spliceforms before and after subsetting the df
df = get_difference(df1)
print("Number of de novo transcripts: ", len(df))

# Remove all the transcripts for which the third column (=difference) is not zero
# For these transcripts at least one of them had a blast hit

df = remove_transcripts(df)
print("Number of de novo transcripts where all alternative splice forms have no hit: " ,len(df))

#Get a list of all transcript ids with all de novo transcripts where all alternative spliceform have no blast hit and the transcripts have a TPM #higher or equal to 0.5
d = remove_TPM(df)
